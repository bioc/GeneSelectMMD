...
"loglkhFunc" <-
function(para, dat, memSubjects, memGenes, eps=1.0e-6)
{
  ...
  tmp<-tapply(X=1:nGenes, INDEX=1:nGenes, FUN=loglkhFunc.default, dat=dat, 
              para=para, memSubjects=memSubjects, memGenes=memGenes, eps=eps)
  res<-sum(tmp)
...
"loglkhFunc.default" <-
function(i, dat, para, memSubjects, memGenes, eps=1.0e-6)
{
  Xi<-dat[i,]
